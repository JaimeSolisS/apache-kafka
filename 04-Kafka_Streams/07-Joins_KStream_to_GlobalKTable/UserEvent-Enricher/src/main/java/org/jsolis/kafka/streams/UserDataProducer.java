...
public class UserDataProducer {
    public static void main(String[] args) throws InterruptedException, ExecutionException {
        ...
        producer.send(purchaseRecord("charlie", "Computer (4)")).get();
        producer.send(userRecord("charlie", "First=charlie,Last=smith,GitHub=abc")).get();
        producer.send(purchaseRecord("charlie", "Books (4)")).get();
        producer.send(userRecord("charlie", null)).get(); // delete for cleanup
